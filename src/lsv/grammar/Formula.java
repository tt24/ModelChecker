...
/**
 * Formula class. Used to verify the model Example: AG( a && b) EF( b || AG (c)
 * ) EF( AG(c) || (b)) Note that if a formula consist of an atomic proposition
 * in the second place, a parenthesis is needed. A( a U AG ( c && b ) )
 */
public class Formula {
	...
	/**
	 * Returns the parents of the FormulaAlt, if the formula is a nested
	 * formula. Example: For AF (p && AG (q) ): > AG (q) is a nested formula.
	 * 
	...
	/**
	 * Returns the negation of the Formula Example: For !EF (c): > negation is
	 * true;
	 * 
	...
	/**
	 * Returns the quantifier/ the temporal of the CTL formula Example: For EG (
	 * a && b ) > quantifier = "EG" For A ( a pUq b ) > quantifier = "A"
	 * 
	...
	/**
	 * Returns the array of the atomic preposition a formula may have. Example:
	 * For AF ( a || c ) > ap = ["a", "c"]
	 * 
	...
	/**
	 * Returns the negation of the formula's atomic preposition Example: For AF
	 * ( !a && b ) > ap = ["a", "b"] > apNeg = [true, false]
	 * 
	 ...
	 * Example: For a > ap = ["a", null] > singleAp = true;
	 * 
	...
	/**
	 * Returns the tautology of a formula Example: For EF (True) > tautology =
	 * ["True", null] For A ( True yUz False ) > tautology = ["True", "False"]
	 * 
	...
	/**
	 * Returns True if a formula only consist of a single tautology (e.g. True,
	 * False)
	 * 
	...
	/**
	 * Returns the nested formula Example: AF( a && AG (c) ) > ap = ["a", null]
	 * , nestedCTL = [null, AG(c)]
	 * 
	...
	/**
	 * Returns the action of a formula. Example: For A( a pUq b ); p = ["act",
	 * "act1"]; q = ["act2", "act3", "act4"] > actions[0] = ["act", "act1"] >
	...
	/**
	 * Returns the operator of a formula Example: For AF ( a && b ) > operator =
	 * "&&" For A ( a pU b ) > operator = "U" For EF ( a || b ) > operator =
	 * "||"
	 * 
